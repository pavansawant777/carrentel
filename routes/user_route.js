...
app.post("/save_login", async function (req, res) {
    ...
    var d = req.body;
    var sql = `SELECT * FROM registration WHERE user_mobile  = '${d.user_mobile}' AND user_password = '${d.user_password}'`;
    var data = await exe(sql);
    if (data.length > 0) {
...
app.get("/",async  function (req, res) {
    ...
    var combinedObj = Object.assign({}, iconsObj, sliderObj,offerobj,offer_second,offer_third);
...
app.get("/proccess_to_book/:id", verify_login, async function(req, res) {
    ...
    const obj = {"services_info": services_info, "pickup_data": pickup_data};
